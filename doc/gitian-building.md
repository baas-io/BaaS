...
*Setup instructions for a gitian build of BaaS using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BaaS
...
Clone the git repositories for BaaS and gitian and then checkout the BaaS version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the BaaS repository
...
To build BaaS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the BaaS repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/baas/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/baas-io/BaaS
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/baas-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/baas-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/baas-build.assert
```
...
[baas/gitian.sigs](https://github.com/baas-io/gitian.sigs/) repository, or if that's not possible to create a pull
request.
