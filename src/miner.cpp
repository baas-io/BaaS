...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
	...
	// Create stake tx
    CMutableTransaction txTempCoinStake;
	if (fProofOfStake) {
		pblock->vtx.push_back(txTempCoinStake);
	}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BaaSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BaaSMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BaaSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("baas-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BaaSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
