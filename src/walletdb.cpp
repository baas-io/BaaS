...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name") {
            ...
            //****check**** false because there is no reason to go through the zerocoin checks for our own wallet
...
bool BackupWallet(const CWallet& wallet, const filesystem::path& strDest)
{
