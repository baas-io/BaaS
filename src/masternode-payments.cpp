...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
	...
	double devfeePercent = pindexPrev->nHeight + 1 > Params().LAST_POW_BLOCK() ? 0.2 : 0.00;
    CAmount blockValue = GetBlockValue(pindexPrev->nHeight);
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue);
	CAmount developerfeePayment = blockValue * devfeePercent;
    ...
    if (hasMasternode) {
        if (fProofOfStake) {
            ...
            txNew.vout[0].nValue = blockValue - masternodePayment - developerfeePayment + nFees;
        }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
	...
	CBitcoinAddress developerfeeaddress(Params().GetDeveloperFeePayee());
    CScript developerfeescriptpubkey = GetScriptForDestination(developerfeeaddress.Get());
        double devfeePercent = nBlockHeight > Params().LAST_POW_BLOCK() ? 0.2 : 0.00;
	...
	bool foundDeveloperPayment = nBlockHeight <= Params().LAST_POW_BLOCK();
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        if (payee.nVotes >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            if (foundMasternodePayment  && foundDeveloperPayment) return true;
