    ...
    vCoins.clear();
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                bool found = false;
                if (nCoinType == ONLY_NOT25000IFMN) {
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-basstake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CBasStake> input(new CBasStake());
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 25000 BAS.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-baas-address
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nFees, true);
...
string CWallet::GetUniqueWalletBackupName() const
{
    ...
    return strprintf("wallet%s.dat", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}