...
#if defined(NDEBUG)
#error "BaaS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uBAS) are considered zero fee (for relaying and mining)
...
bool CheckTransaction(const CTransaction& tx, bool fRejectBadUTXO, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    // Size limits
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE_CURRENT)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if(nSigOps > MAX_TX_SIGOPS_CURRENT)
        return state.DoS(0,
                         error("AcceptToMemoryPool : too many sigops %s, %d > %d",
                               hash.ToString(), nSigOps, MAX_TX_SIGOPS_CURRENT),
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK() && nHeight >= 0) {
        nSubsidy = 625 * COIN;
    } else if (nHeight < 264800 && nHeight >= Params().LAST_POW_BLOCK()) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 527600 && nHeight >= 264800) {
        nSubsidy = 2 * COIN;
    } else if (nHeight < 1053200 && nHeight >= 527600) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < 1578800 && nHeight >= 1053200) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 2630000 && nHeight >= 1578800) {
        nSubsidy = 10 * COIN;
    } else if (nHeight < 4206800 && nHeight >= 2630000) {
        nSubsidy = 15 * COIN;
    } else if (nHeight < 5258000 && nHeight >= 4206800) {
        nSubsidy = 10 * COIN;
    } else if (nHeight < 5783600 && nHeight >= 5258000) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 6309200 && nHeight >= 5783600) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < 6572000 && nHeight >= 6309200) {
        nSubsidy = 2 * COIN;
    } else if (nHeight >= 6572000) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (!tx.IsCoinBase()) {
            ...
            if (!tx.IsCoinStake() && !tx.IsCoinStake())
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> basInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
        	basInputs.push_back(stakeIn);
        }
        const bool hasBASInputs = !basInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBASInputs)
                    ...
                    for (const CTxIn& basIn : basInputs){
                        if(basIn.prevout == in.prevout){
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : basInputs) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ask the first peer connected for them.
